(() => {
  ...
  function applySiteSettings(settings={}) {
    ...
    setText('site-name',site.siteName,DEFAULT_SITE.siteName);setText('hero-title',site.heroTitle,DEFAULT_SITE.heroTitle);setText('hero-text',site.heroText,DEFAULT_SITE.heroText);setText('footer-name',site.siteName,DEFAULT_SITE.siteName);setText('footer-address',site.address,DEFAULT_SITE.address);setText('footer-contact',[site.phone,site.email].filter(Boolean).join(' • '),'Contact information');setText('copyright-name',site.siteName,DEFAULT_SITE.siteName);
  ...
  function mapSupabaseSettings(row) {
    ...
    return {siteName:row.barangay_name||DEFAULT_SITE.siteName,shortName:row.barangay_name||DEFAULT_SITE.shortName,municipality:row.municipality_city||'',province:row.province||'',tagline:locationParts.length?`Official Website • ${locationParts.join(', ')}`:DEFAULT_SITE.tagline,heroTitle:row.hero_title||DEFAULT_SITE.heroTitle,heroText:row.hero_text||DEFAULT_SITE.heroText,address:row.address||DEFAULT_SITE.address,phone:row.contact_number||'',email:row.email||'',logoUrl:row.logo_url||'',theme:{primary:row.primary_color||DEFAULT_SITE.theme.primary,secondary:row.secondary_color||DEFAULT_SITE.theme.secondary,accent:row.accent_color||DEFAULT_SITE.theme.accent}};
  }
  ...
  async function loadRemoteSettings() {
    ...
    const response=await fetch(endpoint,{headers:{apikey:config.publishableKey,Authorization:`Bearer ${config.publishableKey}`,Accept:'application/json'}});if(!response.ok)throw new Error(`Site settings request failed (${response.status}).`);const rows=await response.json();return mapSupabaseSettings(Array.isArray(rows)?rows[0]:null);
  }

  function initYear(){setText('current-year',String(new Date().getFullYear()));}
  document.addEventListener('DOMContentLoaded',async()=>{const fallback=window.BRGYWEB_CONFIG||{};applySiteSettings(fallback);initYear();try{const remote=await loadRemoteSettings();if(remote)applySiteSettings({...fallback,...remote,theme:{...(fallback.theme||{}),...(remote.theme||{})}});}catch(error){console.warn('Using local site settings fallback:',error);}});
