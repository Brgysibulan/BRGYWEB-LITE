(() => {
  ...
  async function loadCurrentLogo() {
    try {
      ...
      if (data?.logo_url) {
        urlInput.value = data.logo_url;
        showPreview(data.logo_url);
      } else {
        showPreview('');
      }
    } catch (error) {
      console.warn('Unable to load logo preview:', error);
    }
  }
  ...
  fileInput.addEventListener('change', () => {
    ...
    if (!ALLOWED_TYPES.has(file.type)) {
      fileInput.value = '';
      setMessage('Logo must be a PNG, JPG, or WebP image.', true);
      showPreview(urlInput.value.trim());
      return;
    }
    ...
    if (file.size > MAX_SIZE) {
      fileInput.value = '';
      setMessage('Logo must be 2 MB or smaller.', true);
      showPreview(urlInput.value.trim());
      return;
    }
    ...
    setMessage('Logo selected. Save Site Settings to upload it.');
  });
  ...
  removeButton?.addEventListener('click', () => {
    ...
    urlInput.value = '';
    showPreview('');
    setMessage('Logo will be removed when you save Site Settings.');
  });
    ...
    try {
      if (selectedFile) {
        ...
        const { error: uploadError } = await client.storage
          .from(BUCKET)
          .upload(OBJECT_PATH, selectedFile, {
            upsert: true,
            cacheControl: '3600',
        ...
        if (!data?.publicUrl) throw new Error('Unable to generate logo URL.');
      ...
      bypassUploadHook = true;
      form.requestSubmit();
      queueMicrotask(() => { bypassUploadHook = false; });
    } catch (error) {
      console.error('Logo upload error:', error);
      setMessage(error.message || 'Unable to update the logo.', true);
