(() => {
  ...
  function readPublic(){return theme.normalizePublic({preset:value('public-preset')||'custom',font:value('public-font'),radius:value('public-radius'),density:value('public-density'),navSkin:value('public-nav-skin'),navPosition:value('public-nav-position'),navAlign:value('public-nav-align'),navMode:value('public-nav-mode'),hero:value('public-hero'),cards:value('public-cards'),contentWidth:value('public-content-width'),colors:readColors()});}
  ...
  function syncResponsiveNote(config){const previewCard=field('public-design-preview')?.closest('.studio3-preview-card');if(!previewCard)return;let note=field('public-responsive-note');if(!note){note=document.createElement('div');note.id='public-responsive-note';note.className='studio3-responsive-note';previewCard.querySelector('.studio3-preview-head')?.insertAdjacentElement('afterend',note);}const effective=typeof theme.effectiveNavPosition==='function'?theme.effectiveNavPosition(config.navPosition):config.navPosition;if(effective===config.navPosition){note.classList.remove('is-fallback');note.textContent=`This device supports the saved ${navLabel(config.navPosition)} navigation.`;}else{note.classList.add('is-fallback');note.textContent=`Desktop setting: ${navLabel(config.navPosition)}. This phone/tablet safely uses Top navigation.`;}}

  function renderPublicPreview(config){const preview=field('public-design-preview');if(!preview)return;const nav=preview.querySelector('.studio3-web-nav'),hero=preview.querySelector('.studio3-web-hero'),body=preview.querySelector('.studio3-web-body'),cards=[...preview.querySelectorAll('.studio3-web-body>div')],c=config.colors;preview.dataset.previewPosition=config.navPosition;preview.dataset.previewMode=config.navMode;preview.dataset.previewAlign=config.navAlign;preview.style.fontFamily=fontValue(config.font);preview.style.setProperty('--preview-primary',c.primary);preview.style.setProperty('--preview-secondary',c.secondary);preview.style.setProperty('--preview-accent',c.accent);preview.style.setProperty('--preview-signal',c.signal);preview.style.setProperty('--preview-surface',c.surface);if(nav){nav.style.background=config.navSkin==='solid'?c.primary:config.navSkin==='glass'?`color-mix(in srgb, ${c.primary} 84%, transparent)`:`linear-gradient(90deg,${c.primary},${c.secondary})`;nav.style.backdropFilter=config.navSkin==='glass'?'blur(14px)':'none';}if(hero){const heroMap={minimal:[`linear-gradient(180deg,${c.surface},#fff)`,'#17201a'],soft:[`linear-gradient(135deg,color-mix(in srgb, ${c.secondary} 14%, white),${c.surface})`,c.primary],split:[`linear-gradient(90deg,${c.primary} 0 58%,${c.secondary} 58% 100%)`,'#fff'],banner:[`linear-gradient(110deg,${c.primary},${c.secondary})`,'#fff'],clean:[c.surface,c.primary],bold:[`linear-gradient(135deg,${c.primary},${c.secondary})`,'#fff']};const [background,color]=heroMap[config.hero]||heroMap.bold;hero.style.background=background;hero.style.color=color;hero.style.borderBottom=config.hero==='banner'?`5px solid ${c.accent}`:'0';hero.style.minHeight=config.hero==='banner'?'112px':config.hero==='minimal'?'120px':config.density==='compact'?'132px':'150px';hero.style.padding=config.contentWidth==='boxed'?'1.25rem 2rem':config.density==='compact'?'1rem':'1.4rem';}if(body){body.style.background=c.surface;body.style.padding=config.contentWidth==='boxed'?'1rem 1.7rem':config.density==='compact'?'.6rem':'.8rem';body.style.gap=config.density==='compact'?'.38rem':'.55rem';}cards.forEach((card,index)=>{card.style.borderRadius=radiusValue(config.radius);card.style.boxShadow=config.cards==='elevated'?'0 7px 18px rgba(17,33,24,.09)':'none';card.style.borderWidth=config.cards==='bordered'?'2px':'1px';card.style.minHeight=config.density==='compact'?'48px':'62px';card.style.padding=config.density==='compact'?'.42rem':'.6rem';card.style.borderColor=config.cards==='bordered'?c.secondary:'#dfe5df';card.style.borderTopColor=index===0?c.accent:index===2?c.signal:card.style.borderColor;card.style.borderTopWidth=index===0?'3px':index===2?'2px':card.style.borderWidth;});syncColorCodes(c);syncResponsiveNote(config);}

  function renderAdminPreview(config){const preview=field('admin-design-preview');if(!preview)return;const side=preview.querySelector('.studio3-admin-side'),main=preview.querySelector('.studio3-admin-main'),panel=preview.querySelector('.studio3-admin-panel'),cards=[...preview.querySelectorAll('.studio3-admin-cards>div')];preview.dataset.previewPreset=config.preset;preview.style.fontFamily=fontValue(config.font);if(side){side.style.background=config.sidebar==='dark'?'linear-gradient(180deg,#20262b,#11161a)':config.sidebar==='light'?'#fff':'linear-gradient(180deg,#0b2f21,#1b6b45)';side.style.color=config.sidebar==='light'?'#17201a':'#fff';side.style.borderRight=config.sidebar==='light'?'1px solid #dfe5df':'0';side.style.padding=config.density==='compact'?'.62rem .52rem':'.85rem .65rem';}if(main)main.style.padding=config.density==='compact'?'.65rem':'.9rem';cards.forEach((card)=>{card.style.borderRadius=radiusValue(config.radius);card.style.boxShadow=config.cards==='elevated'?'0 7px 18px rgba(17,33,24,.09)':'none';card.style.borderWidth=config.cards==='bordered'?'2px':'1px';card.style.minHeight=config.density==='compact'?'56px':'72px';});if(panel){panel.style.borderRadius=radiusValue(config.radius);panel.style.boxShadow=config.cards==='elevated'?'0 7px 18px rgba(17,33,24,.07)':'none';panel.style.borderWidth=config.cards==='bordered'?'2px':'1px';}}
  ...
  function applyPreset(scope,name){if(scope==='public'){const preset=theme.publicPresets[name];if(!preset)return;writePublic(preset);}else{const preset=theme.adminPresets[name];if(!preset)return;writeAdmin(preset);}showWorkspace(scope);refreshPreview(scope);syncDirtyState(true,`${scope==='public'?'Public website':'Admin interface'} preset loaded. Preview updated — save when ready.`);}

  async function loadTheme(){setStatus('Loading saved design…');if(saveButton)saveButton.disabled=true;const {data,error}=await client.from('site_settings').select('design_theme,primary_color,secondary_color,accent_color').eq('id',1).single();if(error)throw error;const config=data?.design_theme||{};const publicConfig={...(config.public||theme.PUBLIC_DEFAULT),colors:{...(config.public?.colors||{}),primary:config.public?.colors?.primary||data.primary_color||theme.COLOR_DEFAULTS.primary,secondary:config.public?.colors?.secondary||data.secondary_color||theme.COLOR_DEFAULTS.secondary,accent:config.public?.colors?.accent||data.accent_color||theme.COLOR_DEFAULTS.accent,signal:config.public?.colors?.signal||theme.COLOR_DEFAULTS.signal}};const cached={...config,public:theme.normalizePublic(publicConfig),admin:theme.normalizeAdmin(config.admin||theme.ADMIN_DEFAULT),version:6};theme.writeCache?.(cached);writePublic(cached.public);writeAdmin(cached.admin);refreshPreview('all');syncDirtyState(false,'Saved design loaded. Choose a preset or change an option to preview it instantly.');}

  async function saveTheme(){
    if(!dirty)return;
    const publicTheme=readPublic(),adminTheme=readAdmin(),payload={version:6,public:publicTheme,admin:adminTheme};
    ...
    try{
      ...
      const saved=data?.design_theme;
      if(!saved?.public||!saved?.admin)throw new Error('Save completed but the saved design could not be verified.');
      const verifiedPublic=theme.normalizePublic(saved.public),verifiedAdmin=theme.normalizeAdmin(saved.admin);
      if(verifiedPublic.font!==publicTheme.font||verifiedPublic.radius!==publicTheme.radius||verifiedPublic.density!==publicTheme.density||verifiedPublic.navSkin!==publicTheme.navSkin||verifiedPublic.navPosition!==publicTheme.navPosition||verifiedPublic.navAlign!==publicTheme.navAlign||verifiedPublic.navMode!==publicTheme.navMode||verifiedPublic.hero!==publicTheme.hero||verifiedPublic.cards!==publicTheme.cards||verifiedPublic.contentWidth!==publicTheme.contentWidth||verifiedAdmin.font!==adminTheme.font||verifiedAdmin.radius!==adminTheme.radius||verifiedAdmin.density!==adminTheme.density||verifiedAdmin.sidebar!==adminTheme.sidebar||verifiedAdmin.cards!==adminTheme.cards)throw new Error('The server returned a design that does not match the selected settings.');
      const verified={version:6,public:verifiedPublic,admin:verifiedAdmin};
      theme.writeCache?.(verified);syncSiteCache(verified);theme.applyAdmin?.(verified.admin);writePublic(verified.public);writeAdmin(verified.admin);refreshPreview('all');syncDirtyState(false,'Published and verified. Public website and admin interface are now using this saved design.');
  ...
  form?.addEventListener('change',(event)=>{if(event.target.matches('select')){const scope=event.target.id.startsWith('public-')?'public':'admin';setValue(`${scope}-preset`,'custom');refreshPreview(scope);syncDirtyState(true,`${scope==='public'?'Public website':'Admin interface'} preview updated. Changes are not published yet.`);}if(event.target.matches('input[type=color]')){setValue('public-preset','custom');refreshPreview('public');syncDirtyState(true,'Brand palette updated in preview. Save to publish these colors.');}});
