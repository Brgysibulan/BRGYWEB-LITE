/**
 * BRGYWEB-LITE — Admin Services
 * Purpose: Manage public barangay service records.
 * Depends on: core/supabase-config.js
 * Used by: admin/services.html
 */
...
(() => {
  ...
  function render() {
    ...
    list.innerHTML = services.map((item) => `<tr><td>${item.sort_order ?? 0}</td><td><strong>${escapeHtml(item.name)}</strong><div class="small text-secondary text-truncate" style="max-width:320px">${escapeHtml(item.description || '')}</div></td><td>${escapeHtml(item.fee_text || '—')}</td><td>${escapeHtml(item.processing_time || '—')}</td><td><span class="badge ${item.is_active ? 'text-bg-success' : 'text-bg-secondary'}">${item.is_active ? 'Published' : 'Hidden'}</span></td><td class="text-end"><div class="d-inline-flex gap-2"><button class="btn btn-sm btn-outline-dark" type="button" data-edit="${item.id}">Edit</button><button class="btn btn-sm btn-outline-danger" type="button" data-delete="${item.id}">Delete</button></div></td></tr>`).join('');
  }
  ...
  if (cachedServices) { services = cachedServices; render(); }
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    const payload = { name: document.getElementById('service-name').value.trim(), description: document.getElementById('service-description').value.trim() || null, requirements: document.getElementById('service-requirements').value.trim() || null, fee_text: document.getElementById('service-fee').value.trim() || null, processing_time: document.getElementById('service-processing').value.trim() || null, sort_order: Number.parseInt(document.getElementById('service-order').value || '0', 10) || 0, is_active: document.getElementById('service-active').checked, updated_at: new Date().toISOString() };
    ...
    try {
      ...
      if (error) throw error;
      resetForm(); setStatus(id ? 'Service updated.' : 'Service added.'); await load();
    } catch (error) { console.error(error); setStatus(error?.message || 'Unable to save service.', true); }
  ...
  list?.addEventListener('click', async (event) => {
    ...
    try {
      ...
      writeCache(services); render(); setStatus('Service deleted.');
      load().catch((refreshError) => console.warn('Services refresh failed:', refreshError));
    } catch (error) { console.error(error); setStatus(error?.message || 'Unable to delete service.', true); deleteButton.disabled = false; }
  });

  // ── Initialization ──────────────────────────────────────────
  requireStaff().then(async (allowed) => {
    if (!allowed) { window.location.href = 'login.html'; return; }
