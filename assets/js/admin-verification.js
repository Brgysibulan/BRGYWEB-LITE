(() => {
  ...
  const clear = () => { Object.values(fields).forEach((el) => { if (!el) return; if (el === fields.status) el.value = 'ACTIVE'; else el.value = ''; }); statusText.textContent = ''; };
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const op = id ? db.from('verification_records').update(payload).eq('id', id) : db.from('verification_records').insert(payload);
    const { error } = await op;
    statusText.textContent = error ? error.message : 'Saved.';
    if (!error) { clear(); await load(); }
  });
  ...
  list.addEventListener('click', async (e) => {
    ...
    if (edit) {
      const r = records.find((x) => String(x.id) === edit.dataset.edit); if (!r) return;
      fields.id.value=r.id; fields.control.value=r.control_number||''; fields.first.value=r.first_name||''; fields.middle.value=r.middle_name||''; fields.last.value=r.last_name||''; fields.designation.value=r.designation||''; fields.acquired.value=r.date_acquired||''; fields.expiration.value=r.expiration_date||''; fields.status.value=r.status||'ACTIVE'; window.scrollTo({top:0,behavior:'smooth'});
    }
  ...
  (async () => { if (!(await ensureAdmin())) { window.location.href='login.html'; return; } await load(); })();
})();
