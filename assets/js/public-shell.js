(() => {
  ...
  const UI_VERSION = '20260901-stability3';
  ...
  function addStyle(key, file) {
    ...
    const link = document.createElement('link'); link.rel='stylesheet'; link.href=`assets/css/${file}?v=${UI_VERSION}`; link.setAttribute(`data-brgy-${key}`,'true'); document.head.appendChild(link);
  }
  ...
  if(header){
    ...
    function setMenuState(open){document.body.classList.toggle('public-menu-open',open);collapseElement?.classList.toggle('show',open);collapseElement?.setAttribute('aria-hidden',open?'false':'true');backdrop.classList.toggle('show',open);backdrop.setAttribute('aria-hidden',open?'false':'true');toggler?.setAttribute('aria-expanded',open?'true':'false');toggler?.setAttribute('aria-label',open?'Close navigation':'Open navigation');}
    function openMenu(){if(isCompactNavigation())setMenuState(true);} function closeMenu(){setMenuState(false);}
    ...
    collapseElement?.querySelectorAll('a[href]').forEach((link)=>link.addEventListener('click',()=>{if(isCompactNavigation())closeMenu();}));
